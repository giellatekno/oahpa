...
import izh_oahpa.settings as settings

from izh_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from izh_oahpa.izh_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
TRANS_CHOICES = (
        ('izhest', u'Ingrian to Estonian'),
        ('estizh', u'Estonian to Ingrian'),
	    #('izheng', _('Ingrian to English')),
	    #('engizh', _('English to Ingrian')),
        ('izhfin', u'Ingrian to Finnish'),
        ('finizh', u'Finnish to Ingrian'),           
        #('izhlat', _('Ingrian to Latvian')),
        #('latizh', _('Latvian to Ingrian')),
        #('izhsme', _('Ingrian to North Saami')),
        #('smeizh', _('North Saami to Ingrian')),
        #('izhnob', _('Ingrian to Norwegian')),
        #('nobizh', _('Norwegian to Ingrian')), 
        #('izhrus', _('Ingrian to Russian')),
        #('rusizh', _('Russian to Ingrian')),
)
...
from izh_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from izh_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
def relax(strict):
	...
	relax_pairs = {
		...
		# The spell-relax rules for Ingrian:
	   u'b́' : u'bʼ',
	   u'b' : u'bʼ',
	   u'd́': u'dʼ',
	   u'd': u'dʼ',
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'izh',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'izh': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'izh', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'izh', 'numlanguage' : 'izh', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'izh', 'numlanguage' : 'izh', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'izh', 'numlanguage' : 'izh', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/izh_oahpa/izh_oahpa/izh_drill/vastaF_log.txt','w')
   ...
   # except socket.timeout:
    ...
    if not language in ["est","lat","fin","eng","rus","izh"]: language="est" # was: nob
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/izh_oahpa/izh_oahpa/izh_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
    ...
    #if language == "fi" : language = "fin"
    #if language == "en" : language = "eng"
    if not language in ["rus","est","fin","eng","lat","izh"]: language="est"
