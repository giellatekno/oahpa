...
class BulkManager(models.Manager):
	...
	def bulk_add_form_messages(self, objs):
		...
		sql = "INSERT %s INTO %s (%s) VALUES %s" % (ignore, "izh_drill_form_feedback", flds, arg_string,)
	...
	def bulk_add_messages(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("izh_drill_feedback_messages", flds, arg_string,)
	...
	def bulk_add_dialects(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("izh_drill_feedback_dialects", flds, arg_string,)
...
def Translations2(target_lang):
	if target_lang in ["nob", "izh", "eng", "dan", "no"]:
		if target_lang == "nob" or "no":	related = 'translations2nob'
		if target_lang == "izh":	related = 'translations2izh'
...
class Word(models.Model):
	...
	language = models.CharField(max_length=5, default='izh', db_index=True)
	...
	def __init__(self, *args, **kwargs):
		...
		self.translations2izh = partial(self.translations2, target_lang='izh')()
