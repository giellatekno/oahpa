...
import vep_oahpa.settings as settings

from vep_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from vep_oahpa.vep_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
TRANS_CHOICES = (
        ('vepest', u'Veps to Estonian'),
        ('estvep', u'Estonian to Veps'),
	    #('vepeng', _('Veps to English')),
	    #('engvep', _('English to Veps')),
        ('vepfin', u'Veps to Finnish'),
        ('finvep', u'Finnish to Veps'),           
        #('veplat', _('Veps to Latvian')),
        #('latvep', _('Latvian to Veps')),
        #('vepsme', _('Veps to North Saami')),
        #('smevep', _('North Saami to Veps')),
        #('vepnob', _('Veps to Norwegian')),
        #('nobvep', _('Norwegian to Veps')), 
        #('veprus', _('Veps to Russian')),
        #('rusvep', _('Russian to Veps')),
)
...
from vep_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from vep_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
def relax(strict):
	...
	relax_pairs = {
		...
		# The spell-relax rules for Veps:
	   u'b́' : u'bʼ',
	   u'b' : u'bʼ',
	   u'd́': u'dʼ',
	   u'd': u'dʼ',
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'vep',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'vep': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'vep', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'vep', 'numlanguage' : 'vep', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'vep', 'numlanguage' : 'vep', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'vep', 'numlanguage' : 'vep', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/vep_oahpa/vep_oahpa/vep_drill/vastaF_log.txt','w')
   ...
   # except socket.timeout:
    ...
    if not language in ["est","lat","fin","eng","rus","vep"]: language="est" # was: nob
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/vep_oahpa/vep_oahpa/vep_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
    ...
    #if language == "fi" : language = "fin"
    #if language == "en" : language = "eng"
    if not language in ["rus","est","fin","eng","lat","vep"]: language="est"
