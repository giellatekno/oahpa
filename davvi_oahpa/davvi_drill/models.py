...
class BulkManager(models.Manager):
    ...
    def bulk_add_form_messages(self, objs):
        ...
        sql = "INSERT %s INTO %s (%s) VALUES %s" % (ignore, "davvi_drill_form_feedback", flds, arg_string,)
    ...
    def bulk_remove_form_messages(self, form_qs):
        ...
        table = "davvi_drill_form_feedback"
        fld = qn('form_id')
    ...
    def bulk_add_messages(self, objs):
        ...
        arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
        sql = "INSERT INTO %s (%s) VALUES %s" % ("davvi_drill_feedback_messages", flds, arg_string,)
    ...
    def bulk_add_dialects(self, objs):
        ...
        arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
        sql = "INSERT INTO %s (%s) VALUES %s" % ("davvi_drill_feedback_dialects", flds, arg_string,)
...
class Word(models.Model):
    ...
    hid = models.IntegerField(null=True, default=None) # max_length=3
