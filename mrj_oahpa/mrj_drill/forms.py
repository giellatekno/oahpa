...
import mrj_oahpa.settings as settings

from mrj_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from mrj_oahpa.mrj_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
from mrj_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from mrj_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'mrj',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'mrj': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'mrj', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'mrj', 'numlanguage' : 'mrj', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'mrj', 'numlanguage' : 'mrj', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'mrj', 'numlanguage' : 'mrj', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/mrj_oahpa/mrj_oahpa/mrj_drill/vastaF_log.txt','w')
   ...
   # except socket.timeout:
    ...
    if not language in ["est","lat","fin","eng","rus","mrj"]: language="est" # was: nob
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/mrj_oahpa/mrj_oahpa/mrj_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
    ...
    #if language == "fi" : language = "fin"
    #if language == "en" : language = "eng"
    if not language in ["rus","est","fin","eng","lat","mrj"]: language="est"
