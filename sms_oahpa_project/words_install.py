...
#scf = importlib.import_module(LLL_OAHPA+'.conf')
...
supported_langs = ['sms', 'rus', 'nob', 'sme', 'fin', 'eng']
	...
	def store_word(self,entry,linginfo,mainlang,paradigmfile,delete,append_only=False):
		...
		if pregenerated:
			...
			for analysis in analyses:
				...
				form, _ = sdm.Form.objects.get_or_create(fullform=f.form, no_show=no_show, tag=tag, word=w)
				form.save()
			...
			if changes_to_paradigm:
				...
				for key, item in paradigms_to_create.iteritems():
					...
					form, _ = sdm.Form.objects.get_or_create(fullform=f.form, no_show=no_show, tag=t, word=w)
					form.save()
