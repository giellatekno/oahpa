...
cat $GTHOME/ped/sme/src/a_smenob.xml | tr "\n" " " | tr "<" "\n" | grep '^l ' | egrep -v '(Ord|none)' | cut -d ">" -f2 | sort -u > t3
...
echo "||  lemma  ||  Attr  ||  Sg Nom ||  Sg Acc ||  Sg Loc || Sg Ill || Comp Attr || Comp Sg Nom || Superl Sg Nom" >> nudoc/gen/sme_adjOahpatabellNorm.jspwiki
...
cat t3|sed 's/$/+A+Sg+Loc/;'| lookup -q $GTHOME/gt/sme/bin/isme-norm.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tsgloc3


cat t3|sed 's/$/+A+Sg+Ill/;'| lookup -q $GTHOME/gt/sme/bin/isme-norm.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tsgill3
