...
# c. oahpa adj
echo "!!!Tabell over Oahpa-adjektiv med GG-dialekt" > nudoc/gen/sme_adjOahpatabellGG.jspwiki
echo "" >> nudoc/gen/sme_adjOahpatabellGG.jspwiki
echo "Testdato: $DATO" >> nudoc/gen/sme_adjOahpatabellGG.jspwiki
echo "||  lemma  ||  Attr  ||  Sg Nom ||  Sg Acc ||  Sg Loc || Sg Ill || Comp Attr || Comp Sg Nom || Superl Sg Nom" >> nudoc/gen/sme_adjOahpatabellGG.jspwiki
...
cat t3|sed 's/$/+A+Sg+Acc/;'| lookup -q $GTHOME/gt/sme/bin/isme-GG.restr.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tsgacc3

cat t3|sed 's/$/+A+Sg+Loc/;'| lookup -q $GTHOME/gt/sme/bin/isme-GG.restr.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tsgloc3

cat t3|sed 's/$/+A+Sg+Ill/;'| lookup -q $GTHOME/gt/sme/bin/isme-GG.restr.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tsgill3

cat t3|sed 's/$/+A+Comp+Attr/;'| lookup -q $GTHOME/gt/sme/bin/isme-GG.restr.fst |tr '\n' '™'|sed 's/™™/£/g;'|tr '£' '\n'|tr '™' '\t'|cut -f2,4,6,8,10,12,14,16|sed 's/.*+A.*/-/'|tr '\t' ',' > tcompattr3
...
cat tadjforms3 >> nudoc/gen/sme_adjOahpatabellGG.jspwiki
