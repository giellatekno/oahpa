...
echo "!!!Tabell over adjektiv - OBS! arbeidsliste for debugging" > smadoc/adjoahpatabell.jspwiki
echo "" >> smadoc/adjoahpatabell.jspwiki
echo " ||  lemma  ||  Attr  ||  Pred  || Sg Nom || Comp Pred || Comp Sg Nom || Superl Pred || Superl Sg Nom " >> smadoc/adjoahpatabell.jspwiki

echo "!!!Reversert tabell over adjektiv - OBS! arbeidsliste for debugging" > smadoc/adjoahpatabellrev.jspwiki
echo "" >> smadoc/adjoahpatabellrev.jspwiki
echo " ||  lemma  ||  Attr  ||  Pred  || Sg Nom || Comp Pred || Comp Sg Nom || Superl Pred || Superl Sg Nom " >> smadoc/adjoahpatabellrev.jspwiki
...
cat tadjforms >> smadoc/adjoahpatabell.jspwiki

rm tattr tpred tsgnom tcomppred tcomp tsuperlpred  tsuperl 

cat t1 | perl -nle 'print scalar reverse $_' > t1rev

paste t1rev tadjforms | sort | cut -f2 | grep '[A-Za-z]' >> smadoc/adjoahpatabellrev.jspwiki
