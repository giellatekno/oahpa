...
#		grade = True and self.settings.get('grade')  or  ""
		...
		if pos == 'N':
			...
			if noun_type == "N-NEUT":
				...
				TAG_QUERY = TAG_QUERY & Q(gender='Msc',animate='Inan')
		...
		try: 
			...
			if case == 'Gen2': # exercise on Genitive2
				if tag.case == 'Gen2':
					WORD_FILTER = WORD_FILTER & Q(word__gen2=True)
				else:  # tag.case = 'Gen'
				    WORD_FILTER = WORD_FILTER & Q(word__gen2=False)"""
				    				
			if noun_type == "N-FEM-8":
				WORD_FILTER = WORD_FILTER & Q(word__inflection_class__contains='8')
			elif noun_type == "N-FEM-other":
				WORD_FILTER = WORD_FILTER & (Q(word__lemma__endswith='а') | Q(word__lemma__endswith='я'))
	...
	def create_form(self, db_info, n, data=None):
		...
		if tag.case in ['Nom', 'Loc2', 'Gen2'] or tag.attributive:
			match_number = False
		else:
			match_number = True
...
class NumGame(Game):
	generate_fst = 'transcriptor-numbers-digit2text.filtered.lookup.xfst' # 'rus-num.fst'
	answers_fst = 'transcriptor-numbers-text2digit.filtered.lookup.xfst' # 'rus-inum.fst'
...
class Klokka(NumGame):
	...
	generate_fst = 'transcriptor-clock-digit2text.filtered.lookup.xfst' # 'iclock-rus.fst'
	answers_fst = 'transcriptor-clock-text2digit.filtered.lookup.xfst' # 'clock-rus.fst'
...
class Dato(Klokka):
	...
	generate_fst = 'transcriptor-date-digit2text.filtered.lookup.xfst' # 'idate-rus.fst'
	answers_fst = 'transcriptor-date-text2digit.filtered.lookup.xfst' # 'date-rus.fst'
