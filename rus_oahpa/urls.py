...
urlpatterns = patterns('',
	...
	url(r'^%s/$' % prefix, 'rusoahpa.rus_drill.views.index'),
	url(r'^%s/i18n/' % prefix, include('django.conf.urls.i18n')),
	url(r'^%s/media/(?P<path>.*)$' % prefix, 'django.views.static.serve',
			  	{'document_root': settings.MEDIA_ROOT}),
	url(r'^%s/courses/' % prefix, include('rusoahpa.courses.urls')),
	url(r'^%s/' % prefix, include('rusoahpa.rus_drill.urls')),
	url(r'^%s/dialect/$' % prefix, 'rusoahpa.conf.views.dialect'),
