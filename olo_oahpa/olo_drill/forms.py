...
import olo_oahpa.settings as settings

from olo_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from olo_oahpa.olo_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
TRANS_CHOICES = (
        ...
        #('latolo', _('Latvian to Livvi')),
        #('olosme', _('Livvi to North Saami')),
...
from olo_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from olo_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
def relax(strict):
	...
	relax_pairs = {
		...
		# The spell-relax rules for Livvi:
	   u'b́' : u'bʼ',
	   u'b' : u'bʼ',
	   u'd́': u'dʼ',
	   u'd': u'dʼ',
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'olo',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'olo': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'olo', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'olo', 'numlanguage' : 'olo', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'olo', 'numlanguage' : 'olo', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'olo', 'numlanguage' : 'olo', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/olo_oahpa/olo_oahpa/olo_drill/vastaF_log.txt','w')
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/olo_oahpa/olo_oahpa/olo_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
