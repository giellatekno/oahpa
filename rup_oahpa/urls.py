...
urlpatterns = patterns('',
	# Example:
	# (r'^rup_oahpa/', include('rup_oahpa.foo.urls')),
	url(r'^%s/$' % prefix, 'rup_oahpa.rup_drill.views.index'),
	url(r'^%s/i18n/' % prefix, include('django.conf.urls.i18n')),
	url(r'^%s/media/(?P<path>.*)$' % prefix, 'django.views.static.serve',
			  	{'document_root': settings.MEDIA_ROOT}),
	url(r'^%s/courses/' % prefix, include('rup_oahpa.courses.urls')),
	url(r'^%s/' % prefix, include('rup_oahpa.rup_drill.urls')),
	url(r'^%s/dialect/$' % prefix, 'rup_oahpa.conf.views.dialect'),
	url(admin_url, include(admin.site.urls)),
	# (r'^admin/doc/', include('django.contrib.admindocs.urls')),
	#url(r'^%s/openid/' % prefix, include('rup_oahpa.openid_provider.urls')),
)
