...
from rup_oahpa.rup_drill.models import *
from rup_oahpa.rup_drill.forms import *
...
try:
	L1 = rup_oahpa.settings.L1
except:
	L1 = 'rup'  # was: sme

try:
	LOOKUP_TOOL = rup_oahpa.settings.LOOKUP_TOOL
except:
	LOOKUP_TOOL = 'lookup'


try:
	FST_DIRECTORY = rup_oahpa.settings.FST_DIRECTORY
except:
	FST_DIRECTORY = False

try:
	DEFAULT_DIALECT = rup_oahpa.settings.DEFAULT_DIALECT
except:
...
#		levels = True and self.settings.get('level')   or   []
		...
		if pos == 'N':
			...
			TAG_QUERY = TAG_QUERY & Q(definite=definite) # added for rup
		...
		if pos == 'Pron':
			sylls = False
			TAG_QUERY = TAG_QUERY & Q(subclass=pron_type) & Q(number__in=['', 'Pl', 'Sg'])  # Sg added by Heli
		elif pos2 == 'Num':
			sylls = False
