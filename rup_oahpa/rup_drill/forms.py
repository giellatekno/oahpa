...
import rup_oahpa.settings as settings

from rup_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from rup_oahpa.rup_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
PRONOUNS_LIST = {'Sg1':u'мон', 'Sg2':u'тон', 'Sg3':u'сон',
		  'Pl1':u'минь', 'Pl2':u'тынь', 'Pl3':u'сынь'}

# DEMONSTRATIVE_PRESENTATION plus Sg3/Pl3
PASSIVE_PRONOUNS_LIST = {'Sg1':u'мон', 'Sg2':u'тон', 'Sg3':u'сон',
		  'Pl1':u'минь', 'Pl2':u'тынь', 'Pl3':u'сынь'}
...
from rup_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from rup_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'rup',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'rup': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'rup', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'rup', 'numlanguage' : 'rup', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'rup', 'numlanguage' : 'rup', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'rup', 'numlanguage' : 'rup', 'numgame': 'string'}
...
class ContextMorfaQuestion(OahpaQuestion):
	...
	qtype_verbs = set(['MAINV', 'V-PRS', 'V-PRT1', 'V-PRT2', 'V-COND','V-IMPRT', 'TEST']) # added MAINV for rup
	...
	def __init__(self, question, qanswer, \
		...
		if answer_tag_el.pos == "N":
			if qtype == "COLL-NUM":
				...
				if answer_tag_el.number=="Sg" or answer_tag_el.number=="SP" or answer_tag_el.case=="Ess" or answer_tag_el.case=="Nom":  #was: qtype="N-NOM-PL" SP added for rup
					self.lemma = answer_word_el.lemma
				else:
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/rup_oahpa/rup_oahpa/rup_drill/vastaF_log.txt','w')
   ...
   # except socket.timeout:
    ...
    if not language in ["nob","sme","fin","eng","swe","rup"]: language="nob"
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/xxxx/xxxxxxxxx/xxxxxxxxx/rup_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
