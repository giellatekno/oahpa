...
class BulkManager(models.Manager):
	...
	def bulk_add_form_messages(self, objs):
		...
		sql = "INSERT %s INTO %s (%s) VALUES %s" % (ignore, "vro_drill_form_feedback", flds, arg_string,)
	...
	def bulk_add_messages(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("vro_drill_feedback_messages", flds, arg_string,)
	...
	def bulk_add_dialects(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("vro_drill_feedback_dialects", flds, arg_string,)
...
def Translations2(target_lang):
	if target_lang in ["nob", "vro", "eng", "dan", "no"]:
		if target_lang == "nob" or "no":	related = 'translations2nob'
		if target_lang == "vro":	related = 'translations2vro'
...
class Word(models.Model):
	...
	reflexive = models.NullBooleanField(blank=True)
	inflection_class = models.CharField(max_length=20) # Zaliznyak's number class
	audio = models.CharField(max_length=70) # audio file name
	...
	def __init__(self, *args, **kwargs):
		...
		self.translations2vro = partial(self.translations2, target_lang='vro')()
