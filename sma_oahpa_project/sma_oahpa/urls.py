...
if oahpa_module.settings.DEV:
	admin_url = r'^admin/(.*)'
else:
	admin_url = r'^%s/admin/(.*)' % prefix

urlpatterns = [
    url(r'^aarjel/$', sdv.index),
	url(r'^%s/i18n/' % prefix, include('django.conf.urls.i18n')),
	url(r'^aarjel/', include(LLL1+'_oahpa.drill.urls')),
    url(r'^%s/media/(?P<path>.*)$' % prefix, serve, {'document_root': MEDIA_ROOT}),
    url(r'^%s/mobile/$' % prefix, scv.set_mobile),
    url(r'^%s/mobile/off/$' % prefix, scv.leave_mobile),
	url(r'^%s/mobil/$' % prefix, scv.set_mobile),
	url(r'^%s/mobil/av/$' % prefix, scv.leave_mobile),
