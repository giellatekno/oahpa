...
class QAGame(Game):
	...
	def generate_answers_subject(self, answer, question, awords, qwords, element="SUBJ"):
		""" Can be used to generate answers for habitive, just supply
		element="HAB".

		This method also performs Question-Answer subject agreement, for
		example:

			Do _you_ like bagels?
			_I_ like bagels.
		...
		# Check whether an answer element should be copied from the question
		# element.
	...
	def generate_syntax(self, answer, question, awords, qwords, s):
		""" Sets up specific syntactic relations between question elements,
	...
	def get_question_morfa(self,db_info,qtype):
		""" This method selects a question object based on the settings form.
		"""
		qwords = {}
	...
	def get_answer_morfa(self,db_info,question):
		""" This compiles the object ids for answer sentences.  There is a lot
		of code in here that is grammatically specifics, e.g., refers to SUBJ,
		HAB, MAINV, because there are special behaviors associated with these
		element names. An update to Morfa-C would probably try to find abstract
		ways to define these relationships, in the event that the software is
		used with languages where there are different syntactic relationships.

		In sme and sma, SUBJ and MAINV have specific syntactic interactions,
		agreement, etc., while HAB and MAINV have separate agreement patterns.
		This function calls others to handle these syntactic patterns.
		"""
	...
	def create_form(self, db_info, n, data=None):
		...
		if not self.gametype == "qa":
			...
			form = (ContextMorfaQuestion(question, answer, \
										 ...
										 db_info['userans'], db_info['correct'], data, prefix=n, user_country=self.settings["user_country"]))
		else:
