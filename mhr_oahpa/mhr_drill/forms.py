...
import mhr_oahpa.settings as settings

from mhr_oahpa.conf.tools import switch_language_code

from models import *
#from game import *
#from mhr_oahpa.mhr_drill.game import relax
import datetime
import socket
import sys, os
import itertools
...
TRANS_CHOICES = (
        ('mhrest', u'Eastern Mari to Estonian'),
        ('estmhr', u'Estonian to Eastern Mari'),
	    #('mhreng', _('Eastern Mari to English')),
	    #('engmhr', _('English to Eastern Mari')),
        ('mhrfin', u'Eastern Mari to Finnish'),
        ('finmhr', u'Finnish to Eastern Mari'),           
        #('mhrlat', _('Eastern Mari to Latvian')),
        #('latmhr', _('Latvian to Eastern Mari')),
        #('mhrsme', _('Eastern Mari to North Saami')),
        #('smemhr', _('North Saami to Eastern Mari')),
        #('mhrnob', _('Eastern Mari to Norwegian')),
        #('nobmhr', _('Norwegian to Eastern Mari')), 
        #('mhrrus', _('Eastern Mari to Russian')),
        #('rusmhr', _('Russian to Eastern Mari')),
)
...
from mhr_oahpa.settings import INFINITIVE_SUBTRACT as infinitives_sub
from mhr_oahpa.settings import INFINITIVE_ADD as infinitives_add
...
def relax(strict):
	...
	relax_pairs = {
		...
		# The spell-relax rules for Eastern Mari:
	   u'b́' : u'bʼ',
	   u'b' : u'bʼ',
	   u'd́': u'dʼ',
	   u'd': u'dʼ',
...
class OahpaSettings(forms.Form):
	...
	def set_default_data(self):
		self.default_data = {
					'language' : 'mhr',  # sme in univ_oahpa
...
class OahpaQuestion(forms.Form):
	...
	def init_variables(self, possible, userans_val, accepted_answers, preferred=False):
		...
		if hasattr(self, 'translang'): # commented out these two lines, because otherwise relax was not working in Morfa
			if self.translang == 'mhr': # caused a problem in Numra, as NumQuestion does not have the attribute translang 
				# Relax spellings.
...
class LeksaSettings(OahpaSettings):
	...
	default_data = {'gametype' : 'leksa', 'language' : 'mhr', 'dialogue' : 'GG',
			#'syll' : [],
			#'bisyllabic': False,
			#'trisyllabic': False,
			#'bisyllabic': False,
...
class NumSettings(OahpaSettings):
	...
	default_data = {'language' : 'mhr', 'numlanguage' : 'mhr', 'dialogue' : 'GG', 'maxnum' : '10', 'numgame': 'numeral'}
...
class KlokkaSettings(NumSettings):
	...
	default_data = {'language' : 'mhr', 'numlanguage' : 'mhr', 'dialogue' : 'GG', 'gametype' : 'kl1', 'numgame': 'string'}
...
class DatoSettings(KlokkaSettings):
	...
	default_data = {'language' : 'mhr', 'numlanguage' : 'mhr', 'numgame': 'string'}
...
def vasta_is_correct(self,question,qwords,language,utterance_name=None):
    ...
    #logfile = open('/home/mhr_oahpa/mhr_oahpa/mhr_drill/vastaF_log.txt','w')
   ...
   # except socket.timeout:
    ...
    if not language in ["est","lat","fin","eng","rus","mhr"]: language="est" # was: nob
...
def cealkka_is_correct(self,question,qwords,awords,language,question_id=None):  #was: question_id=None
    ...
    #logfile = open('/home/mhr_oahpa/mhr_oahpa/mhr_drill/vastas_log.txt', 'w')
    host = 'localhost'
    port = 9000  # was: 9000, TODO - add to settings.py
    size = 1024
    ...
    #if language == "fi" : language = "fin"
    #if language == "en" : language = "eng"
    if not language in ["rus","est","fin","eng","lat","mhr"]: language="est"
