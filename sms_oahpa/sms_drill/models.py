...
class BulkManager(models.Manager):
	...
	def bulk_add_form_messages(self, objs):
		...
		sql = "INSERT %s INTO %s (%s) VALUES %s" % (ignore, "sms_drill_form_feedback", flds, arg_string,)
	...
	def bulk_add_messages(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("sms_drill_feedback_messages", flds, arg_string,)
	...
	def bulk_add_dialects(self, objs):
		...
		arg_string = ', '.join([u'(' + ', '.join(['%s']*len(fields)) + ')'] * len(vals))
		sql = "INSERT INTO %s (%s) VALUES %s" % ("sms_drill_feedback_dialects", flds, arg_string,)
...
def Translations2(target_lang):
	if target_lang in ["nob", "rus", "sme", "eng", "deu", "sms", "no"]:
		...
		if target_lang == "sms":	related = 'translations2sms'
		return related
	else:
		return None
...
class Word(models.Model):
	...
	hid = models.IntegerField(max_length=3, null=True, default=None) # The number of the homonym (1, 2, etc.)
	...
	def __init__(self, *args, **kwargs):
		...
		self.translations2sms = partial(self.translations2, target_lang='sms')()
...
class Form(models.Model):
	...
	def getBaseform(self, match_num=False, return_all=False):
		""" Gets the base form (e.g., citation/dictionary form) for 
			the wordform. Nouns -> Nom+Sg, Verbs -> Inf, Adjectives -> Nom+Sg
