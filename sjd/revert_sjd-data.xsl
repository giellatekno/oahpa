...
<xsl:stylesheet version="2.0"
  ...
  <!-- source language -->
  <xsl:param name="slang" select="'sjd'"/>
  <!-- target language: nob is default! -->
  <xsl:param name="tlang" select="'rus'"/>
  <!-- Output dir, files -->
  <xsl:param name="outDir" select="concat('___', $tlang, '2', $slang)"/>
  ...
  <xsl:template match="/" name="main">
	...
	<r xml:lang="{$tlang}">
		...
		<mg>
		  ...
		  <tg xml:lang="{$slang}">
		    ...
		    <t>
		      ...
		      <xsl:if test="./../../../lg/l_alt">
			<xsl:attribute name="l_alt">
			  <xsl:value-of select="normalize-space(./../../../lg/l_alt)"/>
			</xsl:attribute>
		      </xsl:if>
      ...
      <xsl:result-document href="{$outDir}/{concat(substring-before($current_file, '_sjdrus'), '_', $tlang, $slang)}.{$e}" format="{$of}">
