...
<!--  <xsl:param name="inFile" select="'lemmat_1A'"/> -->
  <xsl:param name="inFile" select="'lemmat_1N'"/>
  ...
  <xsl:template match="/" name="main">
	...
	<lexicon xml:lang="{$slang}">
	  <xsl:for-each select="./lexicon/entry">
	    <xsl:variable name="tr" select="./translations/tr"/>

	    <xsl:if test="not(exists(tokenize($source, $nl)[. = $tr]))">
	      <xsl:copy-of select="."/>
	    </xsl:if>
	    
	    <xsl:if test="exists(tokenize($source, $nl)[. = $tr])">
	      <entry>
		<xsl:copy-of select="./@*"/>
