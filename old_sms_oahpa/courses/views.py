...
def cookie_login(request, next_page=None, required=False, **kwargs):
	...
	if not next_page:
		next_page = '/sms_oahpa/courses/' # TODO: change next url for deep links
	...
	if cookie_uid:
		...
		return HttpResponseRedirect('/sms_oahpa/courses/standard_login/') # TODO: check
...
def cookie_logout(request, next_page=None, **kwargs):
	...
	if not next_page:
		next_page = '/sms_oahpa/courses/logout/'
...
def courses_main(request):
	...
	c = {
        ...
        #return render_to_response(template, c, request)
