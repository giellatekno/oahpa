...
class QAGame(Game):
	...
	def init_tags(self):
		...
		"""
		self.num_fields = 6
		self.syntax =('MAINV','SUBJ','HAB','DUMMY')
	...
	# def generate_answers_dummy(self, answer, question, awords, qwords):
	# 	""" Follows user selection of reciprocative type and returns the relevant
	# 	wordform.
	# 	"""
	...
	def get_answer_morfa(self,db_info,question):
		...
		if db_info.has_key('answer_id') and self.settings['gametype'] == 'context':
			...
			# if 'DUMMY' in words_strings:
			# 	try:
			# 		awords = self.generate_answers_dummy(answer, question, awords, db_info['qwords'])
			# 		print awords
