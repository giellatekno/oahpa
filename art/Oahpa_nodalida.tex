...
<<<<<<< .mine
The web-interface for the OAHPA! platform is implemented in Django, Python-based web development framework combined with a Mysql database.
=======
...
A topic starts with an opening utterance which is either a question or a comment followed by a question. Thus, the user is expected all the time to provide answers to the questions. The dialogue proceeds from to an appropriate utterance inside the current topic. In the end of the topic, there is always a closing comment after which the dialogue proceeds to next topic which is selected according to the answers provided by the user. For example, if the question is about having a car, a positive answer will navigate to a branch with follow-up questions.

The CG-rules are used for navigating inside the dialogue. The input is tagged during analysis with information on whether the answer is interpreted as affirmative or negative. In addition a special tag indicates whether the sentence contains some information that we want to store for the following questions or utterances. The program is thus able to store only simple information such as the student's name, place where she lives and for example the type of her car and use this information in tailored utterances. 

Every utterance contains one or more links to other utterances. The link is selected according to the tag the question-answer pair gets, e.g. \textit{\&dia-neg} or \textit{\&dia-pos}, or \textit{\&dia-target} to a certain word, e.g. target="hivsset", like in Figure \ref{TV}.  In Figure \ref{targetIll} we see how the \textit{\&dia-target} tag is mapped to the noun in illative. \\
...
For example, an answer from the student about her age will induce a tag (Figure \ref{age}), which is used to navigate to different branches of the dialogue, see Figure \ref{branch}.
