...
\frame{\frametitle{Vasta -- QA drill, questions generated from templates} 
...
\frame{\frametitle{Sahka -- dialogue program with precomposed questions} 
...
\frame{\frametitle{The Constraint Grammar parser \textit{vislcg3}}
...
\textit{add, remove, select} or \textit{replace} readings containing tags or sets of grammatical tags in a given sentential context. \pause
...
\item Context conditions in the same rule may be linked, i.e. conditioned upon each other, negated or blocked by interfering baseforms, wordforms or tags.  \\
\end{itemize}
\vspace{0.4cm}
\pause
In 2007, we had a F-measure for Sámi at 95.9, and more developed CG grammars all achieve F-measure for precision and recall in the upper nineties. \\
%\vspace{0.4cm}
...
\frame{\frametitle{Schematical view of the whole process} 
\scalebox{0.57}[0.57]{\includegraphics{img/qa2.pdf}} \\
}


\frame{\frametitle{Morphological analysis} 
\scalebox{0.35}[0.35]{\includegraphics{img/GudelatnjiiQ.png}}\scalebox{0.35}[0.35]{\includegraphics{img/GudelatnjiiA.png}} \\
...
\frame{\frametitle{Evaluation: The actual use of the system}
\begin{itemize}
\item{The programs are freely available at internet since Feb., 2009}
\item{They  get appr. 500 queries/day (not bad for a population of 20000 speakers)}
\end{itemize}\pause
...
If the system has identified an error in the user's input, its response is evaluated as follows:
\begin{table}
\begin{tabular}{|l|c|c|c|}
\hline 
\textbf{Rule type}  & \textbf{correct (tp)} & \textbf{wrong (fp)}   & \textbf{corr. \% }  \\
\hline 
...
\end{tabular}
\end{table}\pause
The main problem for the users is \textit{misspellings}
}

\frame{\frametitle{Evaluation: Some rules are (almost) not in use:}
\begin{itemize}
...
\begin{itemize}
\item The users do not give that elaborate answers  
\item Some of these errors (e.g. PP errors) are not that frequent
\end{itemize}
\pause
...
Running the full log on the present system,  we get the following results:
\begin{table}%[htdp]
%\caption{default}
%\begin{center}
...
\frame{\frametitle{Conclusion}
\begin{itemize}
\item By using a sloppy version of the syntactical CG analyser for North Sámi, combined with a set of error-detection rules, we have been able to build a flexible CALL resource. \\ \pause
\vspace{0.5cm}
\item The present project has shown that CG is well fit for making pedagogical dialogue systems.\\ \pause
\vspace{0.5cm}
\item The program suite is a novelty among pedagogical programs for Sámi, and indeed dialogue and open QA-programs are rare within the field of parser-based CALL.
\end{itemize}
}
...
%How to improve the system? \pause

%\begin{itemize}
%\item{For the misspellings:}
%\begin{itemize}
%\item Integrate our speller to handle misspellings 
%\item Be more tolerant towards the users'  interlanguage
%\end{itemize} \pause
%%\item grammartasks a la \textit{e-tutor}?
%%\item Vasta: decide what words the user should use, ala e-tutor as a supplement
%\item For the unused Vasta program:
%\begin{itemize}
%\item{ let the user choose topic instead of grammar tasks
%\end{itemize} \pause
%\item Overall: Write more detailed feedback
%\item Classroom studies
% \pause
%\item Extend the programs to other (Sámi) languages
%\end{itemize}
%}
...
\frame{\frametitle{Centre for Sámi Language Technology\\ giellatekno.uit.no/ ~ oahpa.uit.no/} 
\begin{center}
...
\scalebox{0.17}[0.17]{\includegraphics{img/Oahpa_graffleposter4d.pdf}} \\
