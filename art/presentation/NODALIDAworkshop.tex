...
\begin{itemize}
\item feedback: "X is not in our lexicon. Could it be a typo?"  \pause
\item What is your name? $\rightarrow$ we accept anything and use the input string: "Good morning, X."  \pause
\item Where do you live? \\
-- placename has not big initial letter \pause $\rightarrow$ fst with placenames with small initial letter and Lowercase-error-tag: "Remember that placenames are written with capital letter."\\  \pause
-- placename is misspelled or it is not in our lexicon  \pause $\rightarrow$ navigation to question: "I haven't heard about X. Is it a place?" -- default: next question. 
\end{itemize}
}
...
\begin{itemize}
\item Loc vs. Nom PxSg3 -- vieljas vs. vielljas (viellja = brother) $\rightarrow$ "Do you mean locative? Remember consonant gradation." 
\item Nom Pl vs. Nom PxSg2 -- vieljat vs. vielljat (viellja = brother) $\rightarrow$ "Do you mean pluralis? Remember consonant gradation." 
...
Some are systematic, some are not: \pause
\begin{itemize}
\item ConNeg -- in bora (borrat = å spise) vs. in borat (boradit = å holde måltid) \\ $\rightarrow$ "You should answer with the same verb as in the question." \\ $\rightarrow$ "The negative form is not correct."  \pause
\item Illative with weak gradation gives V Prt Sg3 -- skuvlii (skuvla N = skole) vs. skuvllii (skuvllet V = å arbeide som lærer)  \pause
\item viesut N Pl Nom (viessu = hus) vs. viessut Imprt Pl1 (viessat = å bli trøtt)
\end{itemize}
}
...
\begin{itemize}
\item vuola:vuollaga vs. vuolla:vuola (øl), in word lists
...
\item "You have not used the appropriate adjective. Try again." \pause
\item The user can quit the dialogue in a proper way (to be implemented)
\end{itemize}
...
\hline
Average  &   3016  &   483   &   3060   &   44   &   0,86   &   0,98   &   0,92   &   0,92			  \\
\hline
\end{tabular}
%\end{center}
%\label{default}
